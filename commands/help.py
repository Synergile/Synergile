...
class Help(commands.Cog, name='Help'):
    ...
    async def help(self, ctx, *, commandName=None):
        if commandName is None:
            ...
            embeds = [discord.Embed(colour=discord.Colour.orange(), title='Help',
                                    timestamp=datetime.now(timezone.utc)).set_thumbnail(
                url='https://i.imgur.com/0NR5nbD.png').set_footer(text=f"Requested by {author}",
                                                                  icon_url=author.avatar_url)]
            count = 0
            for cmd in cmds:
                if (count == 25):
                    embeds.append(discord.Embed(colour=discord.Colour.orange(),
                                                timestamp=datetime.now(timezone.utc)).set_thumbnail(
                        url='https://i.imgur.com/0NR5nbD.png').set_footer(text=f"Requested by {author}",
                                                                          icon_url=author.avatar_url))
                    count = 0
            ...
            if command is None:
                ...
                return
            author = ctx.message.author
            embed = discord.Embed(colour=discord.Colour.orange(), title=command.name,
                                  timestamp=datetime.now(timezone.utc))
            fieldstr = StringBuilder()
            if command.aliases:
                fieldstr.append(f'**Aliases:** {", ".join(command.aliases)}\n')
            fieldstr.append('**Description:** ').append(
                command.description if command.description else 'No description').append('\n')
            fieldstr.append(f'**Usage:** {self.bot.command_prefix}{command.name} ').append(
                command.usage if command.usage else command.signature).append('\n')
            if command.help:
    ...
    async def send_help(self, ctx, content=None, *, tts=False, embed=None, file=None, files=None, delete_after=None,
                        nonce=None):
        return await ctx.send(content=content, tts=tts, embed=embed, file=file, files=files, delete_after=delete_after,
                              nonce=nonce)
