...
def parseTime(timeToParse):
    if isNaN(timeToParse):
        time_string = timeToParse
        if READABLE_TIME_FORMAT_REG.search(time_string):
            ...
            seconds = 0
            milliseconds = 0
            match = YEAR_REG.match(time_string)
            if match is not None:
                years = double(match)
            match = None
            match = WEEK_REG.match(time_string)
            if match is not None:
                weeks = double(match)
            match = None
            match = DAY_REG.match(time_string)
            if match is not None:
                days = double(match)
            match = None
            match = HOUR_REG.match(time_string)
            if match is not None:
                hours = double(match)
            match = None
            match = MIN_REG.match(time_string)
            if match is not None:
                minutes = double(match)
            match = None
            match = SEC_REG.match(time_string)
            if match is not None:
                seconds = double(match)
            match = None
            match = MS_REG.match(time_string)
            if match is not None:
            ...
            time_value = double(milliseconds)/1000
    else:
        time_value = timeToParse
    return time_value